...
Fundamental to localisation is language, so much of the following instruction focuses on providing content in multiple languages. Adapt (v2.0.14) provides two features to support for multiple languages: **Export/Import** and the **Language Picker**.  
...
- As of v2.0.14 Adapt provides grunt tasks for export and import of translatable content. These commands are most easily run when using the Adapt framework as a stand-alone development environment.  
...
As of v2.0.14 Adapt provides grunt tasks for export and import of translatable content.  
...
5. When finished translating, save each file as [utf-8](https://en.wikipedia.org/wiki/UTF-8) in order to accommodate special characters. This is easily done in many word processing applications. It is not - yet - possible within Microsoft® Excel (but this should be [updated](https://excel.uservoice.com/forums/304921-excel-for-windows-desktop-application/suggestions/10006149-support-saving-csv-in-utf-8-encoding) [soon](https://excel.uservoice.com/forums/304933-excel-for-mac/suggestions/9783765-support-utf-8-for-csv-import-export)). A workaround is to open the file in another program such as Notepad and save it as utf-8 from there.  
...
`grunt translate:import --targetLang=de --replace`  
   ...
   **Command options:**  
    ...
    `--format=json|raw|csv` Specifies the format of the files being imported. Acceptable values are `json`, `csv`, or `raw`.  
...
By default all language folders are built with the `grunt build` command. As of v2.0.14 the Adapt framework allows you to specify which languages to build. Use the `--languages` flag to specify an individual language or a subset of languages separated by a comma. To build just the German and French versions of a course, run the following command:   
